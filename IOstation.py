        ...
        for year in years:
            for ID in IDS:
                ...
                if year == years[0] and ID == IDS[0]:
                    ...
                    stTemp, stSalt, stSSH, stUvel, stVvel, stTauX, stTauY = initArrays(years, IDS, deepest,
                                                                                       confM2R.station_names[station],
                                                                                       lon, lat)
...
def getStationIndices(grdObject, st_lon, st_lat, type, numberOfPoints):
    ...
    for i in range(numberOfPoints):
        ...
        if NEG is True:
            print(('Index corresponds to longitude [%3.3f] and latitude [%3.3f]' % (
            longitude[listsIndexes[i][0], listsIndexes[i][1]] - 360, latitude[listsIndexes[i][0], listsIndexes[i][1]])))
        else:
            print(('Index corresponds to longitude [%3.3f] and latitude [%3.3f]' % (
            longitude[listsIndexes[i][0], listsIndexes[i][1]], latitude[listsIndexes[i][0], listsIndexes[i][1]])))
    print('======================')
    print('')
    ...
    for i in range(numberOfPoints):
        dis.append(np.sqrt((latitude[listsIndexes[i][0], listsIndexes[i][1]] - st_lat) ** 2.0 + (
                    longitude[listsIndexes[i][0], listsIndexes[i][1]] - st_lon) ** 2.0))
