...
def calculateVgrid(self):
    print(("---> Setting up vertical coordinates using self.vtransform: %s self.vstretching: %s"
           % (self.vtransform, self.vstretching)))
    if self.vtransform == 1:
        vgrid = s_coordinate(self.h, self.theta_b, self.theta_s, self.tcline, self.nlevels, self.vtransform,
                             self.vstretching, zeta=None)
    elif self.vtransform == 2 and self.vstretching == 2:
        vgrid = s_coordinate_2(self.h, self.theta_b, self.theta_s, self.tcline, self.nlevels, self.vtransform,
                               self.vstretching, zeta=None)
    elif self.vtransform == 2 and self.vstretching == 4:
        vgrid = s_coordinate_4(self.h, self.theta_b, self.theta_s, self.tcline, self.nlevels, self.vtransform,
                               self.vstretching, zeta=None)
    else:
    ...
    def __init__(self, h, theta_b, theta_s, tcline, N, vtransform, vstretching, zeta=None):
        ...
        if self.vtransform == 1:
            if self.tcline > self.hmin:
                warnings.warn(
                    'Vertical transformation parameters are not defined correctly in either gridid.txt or'
        ...
        if self.theta_s >= 0:
            ...
            if self.theta_b >= 0:
                ...
                Cweight = (self.s_rho + self.c1) ** self.Aweight * \
                          (self.c1 + (self.Aweight / self.Bweight) * (self.c1 - (self.s_rho + self.c1) ** self.Bweight))
        ...
        if self.theta_s >= 0:
            ...
            if self.theta_b >= 0:
                ...
                Cweight = (self.s_w + self.c1) ** self.Aweight * \
                          (self.c1 + (self.Aweight / self.Bweight) * (self.c1 - (self.s_w + self.c1) ** self.Bweight))
