...
def createGrid(grdROMS, infilename, outfilename, decimate):
    ...
    print('Old dimensions were  (XI_RHO, ETA_RHO)   : %ix%i' % (
        int(grdROMS.xi_rho) - deltaXI, int(grdROMS.eta_rho) - deltaETA))
    print('Old dimensions were  (XI_U, ETA_U)       : %ix%i' % (
        int(grdROMS.xi_u) - deltaXI, int(grdROMS.eta_u) - deltaETA))
    print('Old dimensions were  (XI_V, ETA_V)       : %ix%i' % (
        int(grdROMS.xi_v) - deltaXI, int(grdROMS.eta_v) - deltaETA))
    print('Old dimensions were  (XI_PSI, ETA_PSI)   : %ix%i' % (
        int(grdROMS.xi_psi) - deltaXI, int(grdROMS.eta_psi) - deltaETA))

    print('New dimensions will be (XI_RHO, ETA_RHO) : %ix%i' % (
        ((int(grdROMS.xi_rho) - deltaXI) / decimate), ((int(grdROMS.eta_rho) - deltaETA) / decimate)))
    print('New dimensions will be (XI_U, ETA_U)     : %ix%i' % (
        ((int(grdROMS.xi_u) - deltaXI) / decimate), ((int(grdROMS.eta_u) - deltaETA) / decimate)))
    print('New dimensions will be (XI_V, ETA_V)     : %ix%i' % (
        ((int(grdROMS.xi_v) - deltaXI) / decimate), ((int(grdROMS.eta_v) - deltaETA) / decimate)))
    print('New dimensions will be (XI_PSI, ETA_PSI) : %ix%i' % (
        ((int(grdROMS.xi_psi) - deltaXI) / decimate), ((int(grdROMS.eta_psi) - deltaETA) / decimate)))
